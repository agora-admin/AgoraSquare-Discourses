    ...
    return (
        <Dialog as='div' open={open} onClose={handleClose}
            ...
            <div className="flex items-center justify-center h-screen backdrop-blur-sm overflow-hidden">
                ...
                <div className={`${open ? 'animate-dEnter': 'animate-dExit'} relative bg-[#141515] border border-[#212427] max-h-[60vh] rounded-2xl max-w-sm w-full mx-5 xs:mx-0 py-4 sm:py-6 gap-4 overflow-hidden`}>
                    {/* Mint Post View */}
                    {<>
                        <Dialog.Title className="text-[#797979] text-sm xs:text-base font-bold tracking-wide flex items-center gap-2 w-max px-6">
                            Fundings
                        </Dialog.Title>
                                    ...
                                    .map((item: any, index: number) => (
                                        <div key={index} className="flex flex-col gap-5 py-2">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <p className="text-gradient text-xs xs:text-sm font-bold">{getFund(item.amount)} {getCurrencyName(chainId)}</p>
